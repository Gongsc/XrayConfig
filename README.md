...
网站顶部日期后显示服务器本次开机以来的已运行时间，每秒更新、每分钟向服务端校准；读取失败时显示暂不可用。

网站顶部新增「IP 质量」标签页，展示基础信息、IP 类型、各来源风险评分、风险因子、流媒体与 AI 解锁、邮件与黑名单六个模块。点击「开始检测」后由 `network-check` 容器检测服务器出口，支持 IPv4、IPv6 和双栈；双栈更新同一份 IPv4、IPv6 报告，不再单独保存或展示双栈报告；单个协议失败不会覆盖另一个协议的报告。导出按钮下载 JSON，包含经脱敏的引擎数据、时间和错误信息。API、页面和导出报告中的 IPv4 只保留前两段，IPv6 只保留前两组；完整出口地址仅在服务内部用于探测。
...
- 同时最多运行一个 IP 检测任务，同协议请求复用任务。每个协议成功结果缓存 5 分钟，失败后等待 60 秒才能重试。脚本每个协议最多运行 5 分钟，随后执行有超时限制的邮件和 DNSBL 探测。
- IPv4、IPv6 独立计算冷却，从各自上一次检测结束起计算，默认 300 秒，同协议在所有访客之间共享。双栈必须等待两者的缓存期和冷却均结束，任一协议受限时不会启动另一协议；双栈检测中每个协议完成后分别开始计时。可在服务器 `.env` 中设置 `IP_QUALITY_MIN_INTERVAL_SECONDS=0..86400`（单位秒，`0` 关闭额外间隔），再执行 `./manage.sh up` 使配置生效；命中限制时接口返回 HTTP 429、`Retry-After` 和可重试时间。读取已有报告不受限制。
...
视觉风格由服务器上的 `site/theme-config.json` 控制。默认配置为 `pixel`，保持当前像素风格；将 `visualTheme` 改为 `glass`，可启用玻璃质感风格：
...
在部署目录修改并保存该文件后，刷新浏览器页面即可生效。Caddy 直接读取挂载的 `site/` 目录，无需重新构建镜像或重启容器。要恢复原风格，把值改回 `pixel`。如果配置文件无法读取或值不受支持，页面会继续使用像素风格。

亮色与暗色由页面右上角的切换按钮控制，两种视觉风格都可使用。访客的选择保存在当前浏览器中；首次访问或没有保存选择时，页面跟随系统亮暗设置。服务器的 `visualTheme` 只控制视觉风格，不会覆盖访客的亮暗选择。关闭 60s 功能时使用的独立静态欢迎页不应用此配置。
...
- `site/theme-config.json`：页面视觉风格的服务器配置文件，使用方法见上文“页面主题切换”。
